...
from .configuration_bert import BertDimPosConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "google-bert/bert-base-uncased"
_CONFIG_FOR_DOC = "BertDimPosConfig"
...
class BertDimPosEmbeddings(nn.Module):
...
class BertDimPosSelfAttention(nn.Module):
...
class BertDimPosAttention(nn.Module):
    def __init__(self, config, layer_idx):
        super().__init__()
        self.self = BertDimPosSelfAttention(config, layer_idx)
        self.output = BertDimPosSelfOutput(config)
        self.num_attention_heads = config.num_attention_heads
...
class BertDimPosOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertDimPosSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertDimPosIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        try:
            ...
            print(f"Error in BertDimPosIntermediate.forward: {e}")
...
class BertDimPosLayer(nn.Module):
    def __init__(self, config, layer_idx):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertDimPosAttention(config, layer_idx)
        self.intermediate = BertDimPosIntermediate(config)
        self.output = BertDimPosOutput(config)
    ...
    def forward(
        ...
        try:
            ...
            print(f"Error in BertDimPosLayer.forward: {e}")
            import traceback
            traceback.print_exc()
            raise
...
class BertDimPosEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([
            BertDimPosLayer(config, layer_idx) 
...
class BertDimPosPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertDimPosPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertDimPosLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = BertDimPosPredictionHeadTransform(config)
...
class BertDimPosOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertDimPosLMPredictionHead(config)
...
class BertDimPosOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertDimPosPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertDimPosLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class BertDimPosPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = BertDimPosConfig
...
@dataclass
class BertDimPosForPreTrainingOutput(ModelOutput):
    """
    Output type of [`BertDimPosForPreTraining`].
...
BERT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`BertDimPosConfig`]): Model configuration class with all the parameters of the model.
...
class BertDimPosModel(BertDimPosPreTrainedModel):
    """
    ...
    _no_split_modules = ["BertDimPosEmbeddings", "BertDimPosLayer"]

    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = BertDimPosEmbeddings(config)
        self.encoder = BertDimPosEncoder(config)

        self.pooler = BertDimPosPooler(config) if add_pooling_layer else None
...
class BertDimPosForPreTraining(BertDimPosPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]

    def __init__(self, config):
        super().__init__(config)

        self.bert = BertDimPosModel(config)
        self.cls = BertDimPosPreTrainingHeads(config)
    ...
    @add_start_docstrings_to_model_forward(BERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=BertDimPosForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], BertDimPosForPreTrainingOutput]:
        r"""
        ...
        ```python
        >>> from transformers import AutoTokenizer, BertDimPosForPreTraining
        >>> import torch

        >>> tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-uncased")
        >>> model = BertDimPosForPreTraining.from_pretrained("google-bert/bert-base-uncased")
        ...
        return BertDimPosForPreTrainingOutput(
            loss=total_loss,
...
class BertDimPosLMHeadModel(BertDimPosPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["cls.predictions.decoder.bias", "cls.predictions.decoder.weight"]

    def __init__(self, config):
        super().__init__(config)

        self.bert = BertDimPosModel(config, add_pooling_layer=False)
        self.cls = BertDimPosOnlyMLMHead(config)
...
class BertDimPosForMaskedLM(BertDimPosPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]
...
class BertDimPosForNextSentencePrediction(BertDimPosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = BertDimPosModel(config)
        self.cls = BertDimPosOnlyNSPHead(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, BertDimPosForNextSentencePrediction
        >>> import torch

        >>> tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-uncased")
        >>> model = BertDimPosForNextSentencePrediction.from_pretrained("google-bert/bert-base-uncased")
...
class BertDimPosForSequenceClassification(BertDimPosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BertDimPosForMultipleChoice(BertDimPosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BertDimPosForTokenClassification(BertDimPosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertDimPosForQuestionAnswering(BertDimPosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
__all__ = [
    "BertDimPosForMaskedLM",
    "BertDimPosForMultipleChoice",
    "BertDimPosForNextSentencePrediction",
    "BertDimPosForPreTraining",
    "BertDimPosForQuestionAnswering",
    "BertDimPosForSequenceClassification",
    "BertDimPosForTokenClassification",
    "BertDimPosLayer",
    "BertDimPosLMHeadModel",
    "BertDimPosModel",
    "BertDimPosPreTrainedModel",
